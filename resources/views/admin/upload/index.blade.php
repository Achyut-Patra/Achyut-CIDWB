...
<!--<script src="{{ url('quickadmin/js') }}/upload_list.js"></script>-->
                ...
                <tbody>
                    @foreach ($upload as $row)
                        <tr>
							...
							<!--<td align="center">
							@if($row->flag==1)
								<img src="{{ asset('img/cidimg/tick.png') }}">
							@else
								<img src="{{ asset('img/cidimg/publish_x.png') }}">
							@endif
							</td>-->
                            <td>
    ...
    $(document).on("click", ".list-inactive", function(e){  //alert("Hii");
     ...
     //alert(innertext);
        $.ajax({
			...
			url: base_url+'/activeinactive/'+idval+'/block',
    ...
    $(document).on("click",".list-active", function(e){  //alert("Naba Hii99");
     ...
     var innertext="<a class='btn btn-default btn-sm list-inactive'  href='unblock/"+idval+"' id='"+idval+"' data-target='#'><i class='fa fa-toggle-off'></i>Inactive</a>";
        ...
        $.ajax({
            ...
            url: base_url+'/activeinactive/'+idval+'/unblock',
            success: function(result){
				//alert(result);
				if(result=='done'){
                	$('#span-'+idval).html(innertext);
